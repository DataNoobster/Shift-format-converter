...
# Function to process the uploaded Excel file with Weaver Id integration
def process_excel_with_runtime_and_output(df: pd.DataFrame):
    try:
        required_columns = {"Weaving Date", "Loom", "Shift", "Base Date and Time", "Pick Counter", "Running Status", "Weaver Id"}
        ...
        df["Weaver Id"] = df["Weaver Id"].astype(str).str.replace('.0', '', regex=False)  # Keep as string, remove decimal
        ...
        for (loom, date), group in df.groupby(["Loom", "Weaving Date"]):
            ...
            # Function to get Weaver Id starting with 91 or 92
            def get_valid_weaver_id(group):
                weaver_ids = group["Weaver Id"].dropna()  # Already strings from preprocessing
                valid_ids = [wid for wid in weaver_ids if wid.startswith(('91', '92'))]
            ...
            if not aa_group.empty:
                ...
                weaver_id_aa = get_valid_weaver_id(aa_group)
            ...
            if not bb_group.empty:
                ...
                weaver_id_bb = get_valid_weaver_id(bb_group)
...
st.markdown('<div class="description">Transform your old shift-wise weaving data into the new shift format with runtime, output, and weaver ID calculations.</div>', unsafe_allow_html=True)
    ...
    with st.spinner("Processing your file..."):
        try:
            ...
            if not processed_df.empty:
                # Ensure Weaver Id is treated as string in the DataFrame
                processed_df["Weaver Id"] = processed_df["Weaver Id"].astype(str).replace('nan', '')
                ...
                with st.expander("View Processed Data", expanded=True):
                    st.dataframe(
                        processed_df.style.format({
                            "Total Output": "{:.0f}",
                            "Weaver Id": lambda x: x if x != '' else 'N/A'  # Format Weaver Id for display
                        }),
                        height=300,
